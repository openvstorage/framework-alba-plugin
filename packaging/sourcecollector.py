    ...
    def collect(target, revision=None, suffix=None):
        ...
        * Any existing tag for repackging 'alpha', 'beta' or 'release' packages
        ...
        if target in ['test', 'stable', 'release']:
            ...
            changelog.append('Open vStorage - ALBA plugin')
            changelog.append('==============================')
            changelog.append('')
        ...
        SourceCollector._run(
            "tar -czf {0}/openvstorage-backend_{1}.tar.gz --transform 's,^,openvstorage-backend-{1}/,' --exclude='*__init__.py' ovs webapps config scripts/install *.txt".format(
