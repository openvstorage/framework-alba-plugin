...
class DebianPackager(object):
    ...
    def package(source_metadata):
        ...
        shutil.copyfile('{0}/openvstorage-backend_{1}.tar.gz'.format(SourceCollector.package_path, version_string),
                        '{0}/openvstorage-backend_{1}.orig.tar.gz'.format(debian_folder, version_string))
        # /<pp>/debian/openvstorage-1.2.3/...
        SourceCollector.run(command='tar -xzf openvstorage-backend_{0}.orig.tar.gz'.format(version_string),
        ...
        SourceCollector.run(command='mv {0}/debian {0}/openvstorage-backend-{1}/'.format(debian_folder, version_string),
                            working_directory=SourceCollector.package_path)

        # Build changelog entry
        with open('{0}/openvstorage-backend-{1}/debian/changelog'.format(debian_folder, version_string), 'w') as changelog_file:
            changelog_file.write("""openvstorage-backend ({0}-1) {1}; urgency=low
...
""".format(version_string, distribution, revision_date.strftime('%a, %d %b %Y %H:%M:%S +0000')))
        ...
        # Some more tweaks
        SourceCollector.run(command='chmod 770 {0}/openvstorage-backend-{1}/debian/rules'.format(debian_folder, version_string),
                            working_directory=SourceCollector.package_path)
        SourceCollector.run(command="sed -i -e 's/__NEW_VERSION__/{0}/' *.*".format(version_string),
                            working_directory='{0}/openvstorage-backend-{1}/debian'.format(debian_folder, version_string))

        # Build the package
        SourceCollector.run(command='dpkg-buildpackage',
                            working_directory='{0}/openvstorage-backend-{1}'.format(debian_folder, version_string))
    ...
    def upload(source_metadata):
        ...
        new_package = version_string not in SourceCollector.run(command='ssh ovs-apt@packages.cloudfounders.com "grep \'openvstorage-backend_{0}-1_amd64\' /data/www/apt/*/Packages" || true'.format(version_string),
                                                                working_directory=SourceCollector.package_path)
        ...
        SourceCollector.run(command='dput -c {0}/debian/dput.cfg ovs-apt {0}/debian/openvstorage-backend_{1}-1_amd64.changes'.format(SourceCollector.package_path, version_string),
                            working_directory=SourceCollector.package_path)
