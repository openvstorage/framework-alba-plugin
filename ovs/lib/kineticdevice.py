...
class KineticDeviceController(object):
    ...
    def discover(interval=0, contents=None):
        """
        ...
        while True:
            ...
            for interface in data['network_interfaces']:
                if 'mac_addr' in interface and 'ipv4_addr' in interface and interface['ipv4_addr'] != '127.0.0.1':
                    ...
                    if not KineticDeviceController.is_valid(interface['ipv4_addr'], interface['mac_addr'], data['port']):
                        continue
                    if key is None:
                        all_data = KineticDeviceController.get_device_info(interface['ipv4_addr'], data['port'])
            ...
            if '_dynamics' in contents or any(c in contents for c in dict_keys):
                for dict_key in dict_keys:
                    if ('_dynamics' in contents or dict_key in contents) and '-{0}'.format(dict_key) not in contents:
    ...
    def get_device_info(ip, port):
        ...
        interfaces = [i for i in information.configuration.interface
                      if hasattr(i, 'MAC') and i.ipv4Address != '127.0.0.1' and KineticDeviceController.is_valid(i.ipv4Address, i.MAC)]
        return {'utilization': dict((u.name, u.value) for u in information.utilization),
                ...
                'statistics': dict((messagetypes[s.messageType].name, {'count': s.count,
                                                    'bytes': s.bytes}) for s in information.statistics),
